...
def admin_panel(lang_code):
    ...
    # the whole section renders disabled. The class→biotope mapping itself is
    # managed directly in the DB (biotope_landcover_map), not in this UI. This is
    # best-effort: any failure leaves the section hidden rather than breaking the
    # admin page.
    gee_available = False
    try:
...
def dashboard(lang_code):
    ...
    try:
        ...
        def _scope(query):
            ...
            # Every query below already joins Location, so the validity rule can
            # be read off the joined row instead of re-running
            # valid_location_id_subquery() as a second scan of the table. Same
            # rule, one scan less per counter.
            query = query.filter(Location.is_valid.is_(True))
            if qc_cond:
